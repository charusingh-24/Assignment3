...
from service_plans.serviceplan_util import generate_api_key
...
def add_user_to_table(username, password,plan):
    # Connect to the database
    conn = sqlite3.connect('meta.db')
    c = conn.cursor()
    tableName = "cred_with_tier"
    # Insert the new user into the table
    c.execute(f"Create TABLE IF NOT EXISTS {tableName} (username,password,plan);")
    c.execute(f"INSERT INTO {tableName} (username, password,plan) VALUES (?, ?,?)", (username, password,plan))
    print("created")
...
#     add_user_to_table('damg7245', pwd,"free")
#
